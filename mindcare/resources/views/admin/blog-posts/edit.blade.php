...
<div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="h3 mb-0">Edit Blog Post</h2>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.blog-posts.update', $blogPost->post_id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('content') is-invalid @enderror" 
                              id="content" name="content" rows="5" required>{{ old('content', $blogPost->content) }}</textarea>
                    @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <select class="form-control @error('user_id') is-invalid @enderror" 
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" 
                                    {{ old('user_id', $blogPost->user_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
